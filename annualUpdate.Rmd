---
title: "Premier League 2016/17 Annual (weekly updates!)"
output: 
  flexdashboard::flex_dashboard
  #vertical_layout: scroll
   # orientation: rows
   # vertical_layout: scroll #fill or scroll
runtime: shiny    
---
...
teamsChoiceCurrent <- standings %>% 
  filter(season=="2017/18") 

teamsChoiceCurrent <- sort(unique(teamsChoiceCurrent$team))
...
sliderInput(
  ...
  position == input$rank_wk1 & season < "2017/18") %>%
...
cardsByGame <-playerGame %>% 
  ...
  mutate(CARD=ifelse(is.na(CARD),"  ",CARD)) %>% 
   mutate(red=ifelse(CARD=="P"|CARD=="R",1,0),yellow=ifelse(CARD=="Y",1,0)) %>% 
    summarise(reds=sum(red,na.rm=0),yellows=sum(yellow,na.rm=0),tot=reds+yellows) 
...
standings %>% 
   ...
   layout(height= 600, autosize=FALSE,  ## height restriction otherwise no x axis
     hovermode = "closest",
     title=theTitle,
     xaxis=list(title=""),
...
### Goals Conceded by Minute by Club

```{r  GA_wk9}

numericInput("conceded_wk9","Select Minute(45 and 90 inc additional", value=1,min=1,max=90,width = '300px')

output$GA_wk9 <- renderPlotly({


  
  theTitle <- paste0("Goals Conceded in Minute ",input$conceded_wk9)
  
  df <- goals %>% 
  filter(TIME==input$conceded_wk9) %>% 
  select(TEAMMATCHID) %>% 
  inner_join(teamGames) %>% 
...
### Player % Goals by Category

```{r  goals_wk9}


sliderInput(
            "pcPlGoals","Minimum Goals",min = 1,max = 250,value = 25
          )
          radioButtons(
...
### Games scoring specific goals per game
...
output$goals_wk10 <- renderPlotly({
  
  theTitle <- paste0("Games with ",input$team_wk10," scoring ",input$gf_wk10,"+ goals")
  
standings %>% 
  group_by(team,season) %>% 
  filter(GF>=input$gf_wk10) %>% 
  tally() %>% 
  arrange(season) %>% 
  filter(team==input$team_wk10) %>% 
...
### Points accrued to date compared with all Seasons/Teams 
...
output$points_wk10 <- renderPlotly({
  
  # print("wk10")
  # print(input$teamA_wk10)
  
  df <- standings %>%
  filter(tmYrGameOrder < 11) %>%
  select(season, team, tmYrGameOrder, cumPts) %>%
  mutate(lineColor = ifelse(team == input$teamA_wk10, "red", "blue")) %>%
  mutate(lineColor = ifelse(team == input$teamA_wk10 &
  season == "2017/18", "grey", lineColor))
  
  # print(glimpse(df))
  # 
  # write_csv(df,"problem.csv")  # works fine
...
selectInput(
  "team_wk11",
  "Select Team",
  choices = teamsChoiceCurrent, 
  selected = "Chelsea",
  multiple = FALSE,
  width = 200
  )

output$GF_gap <- renderPlotly({
a <- teamGames %>% 
  
  filter(TEAMNAME==input$team_wk11&season=="2017/18") %>% 
...
### Average Age of Starting Lineups by Team by Season
...
### Scoring on Debut (click for Players)

```{r  scoreDebut}

data_week12 <- reactive({
...
output$scoreDebut <- DT::renderDataTable({
...
DT::dataTableOutput("scoreDebut")


```

### Players Scoring on Debut

```{r  scoreDebut_details}

output$scoreDebut_details <- DT::renderDataTable({

  print("enter scoreDebut_details ")

  if (is.null(input$scoreDebut_rows_selected)) {
  teamname <- "Tottenham H"

  } else {
  print("enter select")
  s = as.integer(input$scoreDebut_rows_selected)
  
  teamname <-  data_week12()$df[s, ]$team
  }
  
  playerGame %>% 
  filter(plGameOrderApp==1&Gls>0&PLAYERID!="OWNGOAL"&TEAMNAME==teamname) %>% 
  arrange(desc(gameDate)) %>% 
  select(name,date=gameDate,Opps=Opponents)
  
})
DT::dataTableOutput("scoreDebut_details")
...
output$tableToppers <- renderPlotly({
  
  
 theTitle <- paste0("Number of Different Clubs leading Table in first ",input$tableToppers_wk1," games")
  
diffTeams <-standings %>% 
  ungroup() %>% 
  filter(position==1&tmYrGameOrder<=input$tableToppers_wk12) %>% 
  group_by(team,season) %>% 
  tally() %>% 
  ungroup() %>% 
  select(season) %>% 
...
standings %>% 
  ...
  layout(hovermode = "closest",height = 350,
    autosize = F,
...
data_hthTop <- reactive({
topTeams <- standings %>% 
  filter(season==input$season_wk17) %>% 
...
df <-hth %>% 
 # filter(season==input$season_wk17 & team %in% data_hthTop() & OppTeam %in% data_hthTop()) %>% 
  filter(season=="2017/18" & team %in% topTeams & OppTeam %in% topTeams) %>% 
  group_by(team) %>% 
...
return(info)
  ...
  ## top add W/L/D
output$wk_17hthTop <- DT::renderDataTable({
 
 # req(data_hthTop())
  
  print(data_hthTop()$df)
  # print(input$season_wk17)
  # 
  # theSeason <- "2016/17"
  
# hth %>% 
#  # filter(season==input$season_wk17 & team %in% data_hthTop() & OppTeam %in% data_hthTop()) %>% 
#   filter(season=="2016/17" & team %in% data_hthTop()$team & OppTeam %in% data_hthTop()$team) %>% 
#   group_by(team) %>% 
#   summarise(Pl=n(),GF=sum(GF),GA=sum(GA),GD=GF-GA,points=sum(points)) %>% 
#   arrange(desc(points),desc(GD),desc(GF),desc(Pl),team) %>% 
#   select(team,Pl,points,GD) %>%
  ...
  data_hthTop()$df %>% 
                         ...
                         DT::datatable(width=250,class='compact stripe hover row-border order-column',rownames=FALSE,options= list(paging = FALSE, searching = FALSE,info=FALSE, selection="single"))

})

DT::dataTableOutput("wk_17hthTop")
...
# library(timevis)
# 
# actors <- c("Yul Brynner","Steve McQueen","Horst Bucholz","Charles Bronson","Robert Vaughan","Brad Dexter","James Coburn")
# 
# born <- c(1920,1930,1933,1921,1932,1917,1928)
# 
# died <- c(1985,1980,2003,2003,2016,2002,2002)
# 
# df <- tibble(content=actors,start=born,end=died)
# 
# df <- data.frame(content=actors,start=born,end=died)
# df$content <- factor(df$content, levels = df$content[order(-df$start)])
# df <- df %>%
#   arrange(start)
# 
