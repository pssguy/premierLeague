...
# load precompiled data of every player - this could be changed
#playerGame<- readRDS("data/playerGame.rds")

# reduce to just transfer information 
df <- playerGame %>% 
    select(FEE,name,TEAMNAME,joined) %>% 
    unique() %>% 
  # exclude academy talent, free and unknown fees 
    filter(FEE>0&!is.na(FEE)&&FEE!=99)
...
df <-df %>%
   ...
   plot_ly(data=df[which(df$TEAMNAME %in% initial),],x=FEE/1000,y=season,mode="markers",hoverinfo = "text", color=TEAMNAME, colors = "Set3",
     ...
     add_trace((data=df[which(!df$TEAMNAME %in% initial),],x=FEE/1000,y=season,mode="markers",hoverinfo = "text", color=TEAMNAME, colors = "Set3",
           text = paste(name,"<br>",TEAMNAME,"<br>",joined,"<br>",FEE)) %>% 
