  ...
  # might want to look at tidyverse alt e.g odbc/dbi
    ...
    library(ggplot2)
    library(shiny)
    library(RSQLite) # loads DBO for the 
   ...
   # For any existing finished data that just needs to be adjusted 
    conRemote <- dbConnect(odbc::odbc(),
    ...
    # problem this is returned as a list
  ...
  #  [8] "players"     "teams"   MANAGERS AND TEAMS ARE NEW
    ...
    ## need to do this as column name clash casues problem when use in database (need to change some code eg playerDrought)
    #playerGame$off <- NULL
    playerGame$offA <- playerGame$OFF
    playerGame[playerGame$OFF==0,]$offA <- ""
    ...
    # # Error in `[<-.data.frame`(`*tmp*`, standings$GF == standings$GA, , value = list( : 
    ...
    hth<-data.frame(standings %>%
                      select(team,OppTeam:gameDate,venue,points,res,MATCHID,season,GF,GA)) #added GF and GA for hth requirement
   ...
   # if (length(newIds) >0) lw <- rbind(lw,newPlayers) changed mid may 2017
    ...
    ## NB prob should ungroup all but
      ...
      ## having lots of probs with 'opponents' still being ther this works but???
      teamGames <- teamGames %>% ungroup()  ## but still has errors where opponents eg hth drillldown and shows as grouped_df
    ...
    ## look at writing these to a database - initially locally
    
    # Found I needed this first
    playerGame <- playerGame %>% 
      mutate(age=readr::parse_number(age)) #this changes it to double/removes text - should look at 
    ...
    #dbWriteTable(con, "managerTeam", managerTeam ,overwrite = TRUE) #??
    dbWriteTable(con, "allPlayers", allPlayers ,overwrite = TRUE)
    dbWriteTable(con, "playerGame", playerGame,overwrite = TRUE) # has issues whilst 2 columns had simlar names think - but not sure- must have been OFF and off (which I changed to offA)
    dbWriteTable(con, "playerClub", playerClub ,overwrite = TRUE)
    ...
    # then upload remotely from premier to sqlb12webcontrolcenter.com poss using saved premierUpdate script on sqlserver ??
    
    ## do directly
    
    
    
    #dbWriteTable(conRemote, "managerTeam", managerTeam ,overwrite = TRUE) #??
    # dbWriteTable(conRemote, "allPlayers", allPlayers ,overwrite = TRUE)
    # dbWriteTable(conRemote, "playerGame", playerGame,overwrite = TRUE) 
    # dbWriteTable(conRemote, "playerClub", playerClub ,overwrite = TRUE)
    # dbWriteTable(conRemote, "summary", summary ,overwrite = TRUE)
    # dbWriteTable(conRemote, "standings", standings ,overwrite = TRUE)
    # dbWriteTable(conRemote, "leaders", leaders ,overwrite = TRUE)
    # dbWriteTable(conRemote, "allGoalsPlayer", allGoalsPlayer ,overwrite = TRUE)
    # dbWriteTable(conRemote, "goals", goals ,overwrite = TRUE)
    # dbWriteTable(conRemote, "Goals_team_for", Goals_team_for ,overwrite = TRUE)
    # dbWriteTable(conRemote, "Goals_team_ag", Goals_team_ag ,overwrite = TRUE)
    # dbWriteTable(conRemote, "Play", Play ,overwrite = TRUE)
    # dbWriteTable(conRemote, "Method", Method ,overwrite = TRUE)
    # dbWriteTable(conRemote, "Place", Place ,overwrite = TRUE)
    # dbWriteTable(conRemote, "teamGames", teamGames ,overwrite = TRUE)
    # dbWriteTable(conRemote, "milestones", milestones ,overwrite = TRUE)
    # dbWriteTable(conRemote, "managers", managers ,overwrite = TRUE)
    # dbWriteTable(conRemote, "goalSeqs", goalSeqs ,overwrite = TRUE)
    # 
    # 
    # dbWriteTable(conRemote, "goalSeqsClub", goalSeqsClub ,overwrite = TRUE)
    # dbWriteTable(conRemote, "hth", hth ,overwrite = TRUE)
    # dbWriteTable(conRemote, "goalSeqTeam", goalSeqTeam ,overwrite = TRUE)
    
    #There is a problem with many of these on sending DATE field which needs to be datetime - though was not locally??
    
    #tried just doing those 
    dbWriteTable(conRemote, "playerClub", playerClub ,fieldTypes = c(JOINED = "datetime",LEFT = "datetime"),overwrite = TRUE)
    # No error but did not appear when refreshing - but appeared to be ok when reconnecting
    dbWriteTable(conRemote, "summary", summary ,fieldTypes = c(born = "datetime",left = "datetime"),overwrite = TRUE)
    dbWriteTable(conRemote, "standings", standings ,fieldTypes = c(gameDate = "datetime"),overwrite = TRUE)
    dbWriteTable(conRemote, "teamGames", teamGames ,fieldTypes = c(gameDate = "datetime"),overwrite = TRUE)
    dbWriteTable(conRemote, "managers", managers ,fieldTypes = c(Joined = "datetime",Left = "datetime"),overwrite = TRUE)
    dbWriteTable(conRemote, "hth", hth ,fieldTypes = c(gameDate = "datetime"),overwrite = TRUE)
    ## playerGame also had issues but error was shortened -
    #dbWriteTable(conRemote, "playerGame", playerGame ,fieldTypes = c(joined = "datetime",left = "datetime",gameDate = "datetime",born = "datetime"),overwrite = TRUE)
    # might be that age is given $ age                <time> 28.78839 days when it is really years so want as float - corrected above
    ...
    dbWriteTable(conRemote, "goalSeqTeam", goalSeqTeam ,overwrite = TRUE)
    dbWriteTable(conRemote, "playerGeos", playerGeos ,overwrite = TRUE)
